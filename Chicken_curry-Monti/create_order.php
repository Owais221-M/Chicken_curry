...
// =============================================================================
// §3b COUPON / DISCOUNT VALIDATION (server-side re-check)
// =============================================================================

$coupon_discount = 0;
$coupon_info = $input['coupon'] ?? null;

if ($coupon_info && !empty($coupon_info['code'])) {
    $coupon_code = strtoupper(trim($coupon_info['code']));
    $today = date('Y-m-d');
    ...
    $cstmt = $conn->prepare("
        ...
        LIMIT 1
    ");
    $cstmt->bind_param('sss', $coupon_code, $today, $today);
    $cstmt->execute();
    $cresult = $cstmt->get_result();

    if ($cresult->num_rows > 0) {
        $offer = $cresult->fetch_assoc();
        $cart_sub = $total - $SERVICE_FEE - ($order_type === 'delivery' ? $DELIVERY_FEE : 0);

        if ($cart_sub >= floatval($offer['min_order_amount'])) {
            switch ($offer['discount_type']) {
                case 'percentage':
                    $coupon_discount = round($cart_sub * ($offer['discount_value'] / 100), 2);
                    break;
                case 'fixed':
                    $coupon_discount = min(floatval($offer['discount_value']), $cart_sub);
                    break;
            }
        }
    }
...
$stripe_secret_key = getenv('STRIPE_SECRET_KEY') ?: 'sk_test_YOUR_SECRET_KEY_HERE'; // Replace with real key or set env var
...
$stmt = $conn->prepare('INSERT INTO orders (customer_name, customer_phone, customer_email, total_amount, order_type, notes) VALUES (?, ?, ?, ?, ?, ?)');
$stmt->bind_param('sssdss', $name, $phone, $email, $total, $order_type, $notes);
$stmt->execute();
